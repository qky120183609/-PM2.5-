...
plots = [
    {
        "sql": "select avg_pm from tb_daily_stats where city_id = 3 and avg_pm is not null",
        "col": "avg_pm",
        "title": "成都整体",
        "ax": axes[0]
    },
    {
        "sql": "select avg_pm_cn1 from tb_daily_stats where city_id = 3 and avg_pm_cn1 is not null",
        "col": "avg_pm_cn1",
        "title": "草堂寺",
        "ax": axes[1]
    },
    {
        "sql": "select avg_pm_cn2 from tb_daily_stats where city_id = 3 and avg_pm_cn2 is not null",
        "col": "avg_pm_cn2",
        "title": "沙河浦",
        "ax": axes[2]
    },
    {
        "sql": "select avg_pm_us from tb_daily_stats where city_id = 3 and avg_pm_us is not null",
        "col": "avg_pm_us",
        "title": "美国大使馆",
        "ax": axes[3]
    },
...
plt.suptitle('成都市各站点PM2.5等级占比饼图', fontsize=18)
plt.tight_layout()
import sys

...
plots = [
    {
        "sql": "select avg_pm from tb_daily_stats where city_id = 2 and avg_pm is not null",
        "col": "avg_pm",
        "title": "上海整体",
        "ax": axes[0]
    },
    {
        "sql": "select avg_pm_cn1 from tb_daily_stats where city_id = 2 and avg_pm_cn1 is not null",
        "col": "avg_pm_cn1",
        "title": "静安区",
        "ax": axes[1]
    },
    {
        "sql": "select avg_pm_cn2 from tb_daily_stats where city_id = 2 and avg_pm_cn2 is not null",
        "col": "avg_pm_cn2",
        "title": "徐汇区",
        "ax": axes[2]
    },
    {
        "sql": "select avg_pm_us from tb_daily_stats where city_id = 2 and avg_pm_us is not null",
        "col": "avg_pm_us",
        "title": "美国大使馆",
        "ax": axes[3]
    },
...
plt.suptitle('上海市各站点PM2.5等级占比饼图', fontsize=18)
plt.tight_layout()
import sys

...
select 
    ...
    avg(avg_pm_cn1) as jingan,
    avg(avg_pm_cn2) as xuhui,
    avg(avg_pm_us) as us
from tb_daily_stats
where city_id = 2
...
ax.plot(months, df['jingan'], marker='o', label='静安区', linewidth=2)
ax.plot(months, df['xuhui'], marker='s', label='徐汇区', linewidth=2)

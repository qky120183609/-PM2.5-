...
select 
    ...
    avg(avg_pm_cn1) as caotangsi,
    avg(avg_pm_cn2) as shahepu,
    avg(avg_pm_us) as us
from tb_daily_stats
where city_id = 3
...
ax.plot(months, df['caotangsi'], marker='o', label='草堂寺', linewidth=2)
ax.plot(months, df['shahepu'], marker='s', label='沙河浦', linewidth=2)

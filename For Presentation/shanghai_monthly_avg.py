...
# 筛选上海数据
df_sh = df_all[df_all["city_id"] == 2]

# 筛选日期范围 2013-2015
df_sh = df_sh[(df_sh['stat_date'] >= '2013-01-01') & (df_sh['stat_date'] <= '2015-12-31')]

# 按年月分组计算月均值
df_sh['year'] = df_sh['stat_date'].dt.year
df_sh['month'] = df_sh['stat_date'].dt.month
monthly = df_sh.groupby(['year', 'month']).agg({
    'avg_pm_cn1': 'mean',
    'avg_pm_cn2': 'mean',
    'avg_pm_us': 'mean'
}).reset_index()
...
ax.plot(months, monthly['avg_pm_cn1'], marker='o', label='静安区', linewidth=2)
ax.plot(months, monthly['avg_pm_cn2'], marker='s', label='徐汇区', linewidth=2)
ax.plot(months, monthly['avg_pm_us'], marker='d', label='美国大使馆', linewidth=2)

ax.set_xlabel('时间')
ax.set_ylabel('PM2.5浓度 (μg/m³)')
ax.set_title('上海市月均PM2.5指数 (2013-2015)')
